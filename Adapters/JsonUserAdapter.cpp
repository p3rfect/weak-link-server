...
User JsonUserAdapter::get_user_from_json(Json::Value root) {
    User user{(root.isMember("id") ? root["id"].asInt() : 0), root["username"].asString(), root["password"].asString(),
...
Json::Value JsonUserAdapter::get_json_from_user(User user) {
    ...
    root["picture"]["file"] = ImageService::encode_image(user.get_path_to_pic());
    return root;
}